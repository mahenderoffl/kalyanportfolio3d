  ...
  return (
    <>
        ...
        <nav className="container-max flex items-center justify-between h-16 md:h-20">
          ...
          >
            <div className="w-6 h-5 flex flex-col justify-between">
              <span
                className={`w-full h-0.5 bg-[var(--text-primary)] rounded-full origin-left transition-transform duration-200 ${
                  isOpen ? "rotate-45 translate-y-[1px]" : ""
                }`}
              />
              <span
                className={`w-full h-0.5 bg-[var(--text-primary)] rounded-full transition-all duration-200 ${
      ...
      >
        ...
        {/* Menu Content */}
        <nav
          className={`absolute right-0 top-0 bottom-0 w-full max-w-sm bg-[var(--bg-secondary)] border-l border-[var(--glass-border)] flex flex-col pt-24 px-8 transition-transform duration-300 ${
          ...
          <div className="flex flex-col gap-2">
            {navLinks.map((link, index) => (
              <button
                key={link.href}
                onClick={() => handleNavClick(link.href)}
                className={`text-left px-4 py-3 text-lg font-medium rounded-xl transition-all duration-150 ${
                  activeSection === link.href.replace("#", "")
          ...
          <div className="mt-8 flex flex-col gap-3">
            ...
            >
              ...
              <div className="mt-auto pb-8 flex items-center justify-center gap-4">
                <a
                  href="https://github.com/mahenderbanoth"
                  target="_blank"
                ...
                <a
                  href="mailto:contact@mahender.dev"
                  className="btn-icon"
                >
                  <svg
                    className="w-5 h-5"
      ...
      <div
        className="fixed top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-[var(--accent-purple)] via-[var(--accent-blue)] to-[var(--accent-cyan)] origin-left z-[60]"
